...
import { packBlockKey } from '../world/blockKey'
...
export function assertMiningSystemSmoke() {
  ...
  const stoneKey = packBlockKey(1, 2, 3)
  if (!session.begin({ key: stoneKey, id: 'stone', durationMs: 600 }, 1_000)) {
    throw new Error('Mining system smoke failed: valid targets should begin')
  }
  const first = session.update(1_300, stoneKey)
  const reused = session.update(1_599, stoneKey)
  ...
  const completed = session.update(1_600, stoneKey)
  if (completed.status !== 'complete' || completed.key !== stoneKey || session.active) {
    throw new Error('Mining system smoke failed: matching targets should complete exactly once')
  }
  if (session.update(1_700, stoneKey).status !== 'idle') {
    throw new Error('Mining system smoke failed: completed sessions should not repeat')
  }

  session.begin({ key: packBlockKey(4, 5, 6), id: 'wood', durationMs: 400 }, 2_000)
  const cancelled = session.update(2_100, packBlockKey(7, 8, 9))
  if (cancelled.status !== 'cancelled' || session.active) {
    throw new Error('Mining system smoke failed: changing aim should cancel mining')
  }
  if (session.begin({ key: -1, id: 'wood', durationMs: 0 }, Number.NaN)) {
